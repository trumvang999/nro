...
function fetchProductInfo() {
  const productName = document.querySelector('#ttin .post-title-02 a')?.innerText || "Không rõ sản phẩm";
  const priceRaw = document.querySelector('.card')?.innerText || "0đ";
  productPrice = parseInt(priceRaw.replace(/[^\d]/g, ''), 10) || 0;

  // Gán vào chuyển khoản
  document.getElementById('display-amount').innerText = productPrice.toLocaleString() + " VNĐ";
  document.getElementById('display-content').innerText = `${productName}`;

  // Gán vào tab thẻ cào
  document.getElementById('original-price').innerText = productPrice.toLocaleString() + " VNĐ";
  const increased = productPrice * 1.25;
  const rounded = roundUpTo10k(increased);
  document.getElementById('card-price').innerText = rounded.toLocaleString() + " VNĐ";
}
...
function showTab(tabId) {
  ...
  if (tabId === 'bank-tab') buttons[0].classList.add('active');
  else buttons[1].classList.add('active');

}

// Reset popup về mặc định
function resetPaymentPopup() {
  // Ẩn bảng chuyển khoản
  document.getElementById('payment-info').style.display = 'none';

  // Mặc định hiển thị tab chuyển khoản
  document.getElementById('bank-tab').style.display = 'block';
  document.getElementById('card-tab').style.display = 'block';
  document.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('active'));
  document.querySelectorAll('.tab-btn')[0].classList.add('active');

  // Khôi phục nút về Đồng ý + Đóng
  document.getElementById('button-area').innerHTML = `
    <button class='btn btn-primary' id='confirm-btn' style='background:#00b000;display:none;'>Đồng ý</button>
...
// Khi mở popup hoặc nạp lại trang
fetchProductInfo();
resetPaymentPopup();
</script>
<script>
